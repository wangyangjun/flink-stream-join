...
public class StreamJoinOperator<K, IN1, IN2, OUT>
    ...
    public void processElement1(StreamRecord<IN1> element) throws Exception {
        ...
        if (setProcessingTime) {
            ...
            for (StreamRecord<IN2> record2 : stream2Buffer.getElements()) {
                ...
                if (time2 + this.stream2WindowLength >= time1) {
    ...
    public void processElement2(StreamRecord<IN2> element) throws Exception {
        ...
        if (setProcessingTime) {
            ...
            for (StreamRecord<IN1> record1 : stream1Buffer.getElements()) {
                ...
                if (time1 + this.stream1WindowLength >= time2) {
                    if (keySelector1.getKey(item1).equals(keySelector2.getKey(item2)))
                        output.collect(new StreamRecord<>(userFunction.join(item1, item2)));
                } else {
                    expiredDataNum++;
    ...
    public void processWatermark1(Watermark mark) throws Exception {
        ...
        for (StreamRecord<IN1> record1 : stream1Buffer.getElements()) {
            if(record1.getTimestamp() >= this.currentWatermark1
                    && record1.getTimestamp() < mark.getTimestamp()){
                for (StreamRecord<IN2> record2 : stream2Buffer.getElements()) {
                    if (record2.getTimestamp() + this.stream2WindowLength >= record1.getTimestamp()) {
        ...
        int expiredDataNum = 0;
        for (StreamRecord<IN2> record2 : stream2Buffer.getElements()) {
            if (record2.getTimestamp() + this.stream2WindowLength < mark.getTimestamp()) {
                expiredDataNum++;
            } else
                break;
        }
    ...
    public void processWatermark2(Watermark mark) throws Exception {
        // process elements after current watermark2 and lower than mark
        for (StreamRecord<IN2> record2 : stream2Buffer.getElements()) {
            if(record2.getTimestamp() >= this.currentWatermark1
                    && record2.getTimestamp() < mark.getTimestamp()){
                for (StreamRecord<IN1> record1 : stream1Buffer.getElements()) {
                    if (record1.getTimestamp() + this.stream1WindowLength >= record2.getTimestamp()) {
                        output.collect(new StreamRecord<>(userFunction.join(record1.getValue(), record2.getValue())));
                    }
                }
            }
        ...
        int expiredDataNum = 0;
        for (StreamRecord<IN1> record1 : stream1Buffer.getElements()) {
            if (record1.getTimestamp() + this.stream1WindowLength < mark.getTimestamp()) {
                expiredDataNum++;
            } else
                break;
        }
